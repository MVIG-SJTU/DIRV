...
from utils.visual import visual_demo
...
ap.add_argument('-w', '--weights', type=str, default='weights/vcoco_best.pth', help='/path/to/weights')
ap.add_argument('--image_path', type=str, default='test/test.jpg', help='/path/to/image')
ap.add_argument('--save_path', type=str, default='test/detection.jpg', help='/path/to/detection/result')
...
print(f'running demonstration with weights {weights_path} on image {image_path}...')
...
def hoi_match(preds_inst, preds_union, prior_mask, thre=0.05, human_thre=0.6, anchor_thre=0.25, loc_thre=0.1):
...
def img_detect(file, model, input_size, regressBoxes, clipBoxes, prior_mask, threshold):
    img_path = file
    ...
    with torch.no_grad():
        ...
        dets = hoi_match(preds_inst, preds_union, prior_mask)
...
def test(threshold=0.2):
    ...
    img_detection = img_detect(image_path, model, input_size, regressBoxes, clipBoxes, prior_mask, threshold=threshold)
    visual_demo(img_detection, image_path, save_path)
